...
export default function Home() {
  ...
  const bgRef = useHeroAnimation();
  const [productsRef, productsVis] = useScrollReveal({ threshold: 0.05 });
  ...
  return (
    <div>
      ...
      <section className="position-relative overflow-hidden" style={{ height: 'calc(100vh + var(--header-height))', minHeight: 'calc(680px + var(--header-height))', marginTop: 'calc(var(--header-height) * -1)' }}>
        <div className="hero-bg" style={{ backgroundColor: '#f0e8e8' }}>
          <img ref={bgRef} src="/hero-bg.jpg" alt="" className="hero-bg__img" />
        </div>
          ...
          <div className="row w-100">
            <div className="col-lg-7">
              ...
              <h1 className="animate-in animate-in--d1 mb-4" style={{ color: '#fff', fontFamily: "'Playfair Display', Georgia, serif", fontSize: 'clamp(2rem, 4.8vw, 3.5rem)', fontWeight: 300, fontStyle: 'italic', lineHeight: 1.35, letterSpacing: '3px', textShadow: '0 2px 30px rgba(0,0,0,0.12)' }}>
                Elegance in<br /><span className="hero-every">Every</span> Thread
              </h1>
              <p className="animate-in animate-in--d2 mb-5" style={{ color: 'rgba(255,255,255,0.7)', fontSize: 'clamp(0.9rem, 2vw, 1rem)', maxWidth: '480px', lineHeight: 1.7, fontWeight: 300, fontFamily: "'Playfair Display', Georgia, serif", fontStyle: 'italic', letterSpacing: '0.5px' }}>
      ...
      <div className="ds-container section-premium products-section">
        ...
        {!loading && !error && (
          <div ref={productsRef} className={`row g-4 mx-0 stagger ${productsVis ? 'is-visible' : ''}`}>
